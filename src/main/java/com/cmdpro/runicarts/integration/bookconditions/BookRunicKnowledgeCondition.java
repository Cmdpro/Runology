...
public class BookRunicKnowledgeCondition extends BookCondition {
    ...
    public List<Component> getTooltip(BookConditionContext context) {
        ...
        list.add(Component.translatable("book.runicarts.condition.runicknowledge.ln1", runicKnowledge));
        list.add(Component.translatable("book.runicarts.condition.runicknowledge.ln2", ClientPlayerData.getPlayerRunicKnowledge()));
        if (hasAdvancement) {
            list.add(Component.translatable("book.runicarts.condition.runicknowledge.ln3", Component.translatable(makeDescriptionId("advancements", advancementId) + ".title")));
        }
        return list;
    }
