...
public class RunologyCoreShaders {
    ...
    public static void shaderRegistry(RegisterShadersEvent event) throws IOException {
        event.registerShader(new ShaderInstance(event.getResourceProvider(), Runology.locate("shatter"), DefaultVertexFormat.POSITION), shader -> { SHATTER = shader; });
        event.registerShader(new ShaderInstance(event.getResourceProvider(), Runology.locate("shatter_particle"), DefaultVertexFormat.PARTICLE), shader -> { SHATTER_PARTICLE = shader; });
    }
}
