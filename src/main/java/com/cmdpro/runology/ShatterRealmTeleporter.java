...
public class ShatterRealmTeleporter implements ITeleporter {
    ...
    public @Nullable PortalInfo getPortalInfo(Entity entity, ServerLevel destWorld, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        ...
        if (destWorld.dimension().equals(Level.OVERWORLD)) {
            pos = new Vec3(pos.x, destWorld.getMaxBuildHeight(), pos.z);
        }
        Vec3 pos2 = pos;
        boolean found = false;
        while (pos2.y > destWorld.getMinBuildHeight() && !destWorld.getBlockState(BlockPos.containing(pos2)).isSolid()) {
            pos2 = pos2.add(0, -1, 0);
            if (destWorld.getBlockState(BlockPos.containing(pos2)).isSolid()) {
                found = true;
                pos = pos2.add(0, 1, 0);
            }
        }
