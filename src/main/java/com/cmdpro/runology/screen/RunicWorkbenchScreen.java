        ...
        if (pMouseX >= x+124 && pMouseY >= y+12 && pMouseX <= x+139 && pMouseY <= y+27) {
            if (menu.blockEntity.runicEnergyCost.size() > 0) {
                ...
                for (Map.Entry<String, Float> i : menu.blockEntity.runicEnergyCost.entrySet()) {
                    tooltip.add(Component.translatable("container.runology.runicworkbench.runicenergycost", i.getValue(), Component.translatable(Util.makeDescriptionId("rune", ResourceLocation.tryParse(i.getKey())))).getVisualOrderText());
                }
